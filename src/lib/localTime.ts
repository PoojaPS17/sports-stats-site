...
function clockText(d: Date, opts: { clock24?: boolean; timeZone?: string }): string {
  const { clock24, timeZone } = opts;
...
export function formatLocalTime(iso: string, fmt: LocalTimeFormat, opts: { clock24?: boolean; timeZone?: string } = {}): string {
