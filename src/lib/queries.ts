...
const GAME_SELECT = `
  select
    g.espn_id, g.date, g.name, g.short_name, g.home_score, g.away_score,
    g.status_state, g.status_detail, g.completed,
    g.home_team_espn_id, g.away_team_espn_id,
    ht.name as home_name, ht.slug as home_slug, ht.abbreviation as home_abbr, ht.logo_url as home_logo,
    at.name as away_name, at.slug as away_slug, at.abbreviation as away_abbr, at.logo_url as away_logo
  from games g
...
export async function getStandings(league: League): Promise<StandingRow[]> {
  const { rows } = await pool.query(
    `select s.team_espn_id, t.name, t.slug, t.abbreviation, t.logo_url,
...
export async function getTeamBySlug(league: League, slug: string): Promise<TeamRow | null> {
  const { rows } = await pool.query(
    `select espn_id, name, slug, abbreviation, logo_url from teams where league = $1 and slug = $2`,
    [league, slug]
  );
  return rows[0] ?? null;
}
...
export async function getAllTeams(league: League): Promise<TeamRow[]> {
  const { rows } = await pool.query(
    `select espn_id, name, slug, abbreviation, logo_url from teams where league = $1 order by name`,
    [league]
  );
  return rows;
}
