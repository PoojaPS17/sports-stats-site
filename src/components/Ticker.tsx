...
export function Ticker({ items, updatedAt }: { items: TickerItem[]; updatedAt: string | null }) {
  ...
  return (
    <div className="flex items-center gap-4 border-b border-[var(--border)] bg-[var(--text)] py-2 pr-4 text-[var(--bg)]">
      <div className="flex-1 overflow-hidden">
        {items.length > 0 && (
          <div className="flex w-max animate-marquee gap-8 whitespace-nowrap pl-4 text-xs font-medium">
            {doubled.map((item, i) => (
