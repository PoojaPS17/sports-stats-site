...
export function LegalPage({ title, updated, subtitle, children }: { title: string; updated?: string; subtitle?: string; children: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-6">
      <Breadcrumbs items={[{ label: title }]} />
      <PageHeader title={title} subtitle={subtitle ?? (updated ? `Last updated ${updated}` : undefined)} />
