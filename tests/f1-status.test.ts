...
test("f1EventStatus: a race with a winner is a result", () => {
  assert.deepEqual(f1EventStatus({ winner_name: "Max Verstappen", race_completed: true, race_status_detail: "Final" }), { kind: "result", label: null });
});

test("f1EventStatus: a Grand Prix ESPN cancelled is called off, whatever the calendar says", () => {
  assert.deepEqual(f1EventStatus({ winner_name: null, race_completed: false, race_status_detail: "Canceled" }), { kind: "called-off", label: "Cancelled" });
  assert.deepEqual(f1EventStatus({ winner_name: null, race_completed: false, race_status_detail: "Postponed" }), { kind: "called-off", label: "Postponed" });
  assert.deepEqual(f1EventStatus({ winner_name: null, race_completed: null, race_status_detail: "Canceled" }), { kind: "called-off", label: "Cancelled" });
});

test("f1EventStatus: a scheduled race is upcoming, and so is one with no race session on file", () => {
  assert.deepEqual(f1EventStatus({ winner_name: null, race_completed: false, race_status_detail: "Sat, September 26th at 7:00 AM EDT" }), { kind: "upcoming", label: null });
  assert.deepEqual(f1EventStatus({ winner_name: null, race_completed: null, race_status_detail: null }), { kind: "upcoming", label: null });
});

test("f1EventStatus: a finished race is never called off, even with no winner on file", () => {
  assert.deepEqual(f1EventStatus({ winner_name: null, race_completed: true, race_status_detail: "Final" }), { kind: "upcoming", label: null });
  assert.equal(f1EventStatus({ winner_name: null, race_completed: true, race_status_detail: "Abandoned" }).kind, "upcoming");
});
...
test("f1EventDescription: a result names the winner, a called-off Grand Prix says so, a future one says results are added as sessions finish", () => {
  const base = { name: "Gulf Air Bahrain Grand Prix", race_completed: false as boolean | null };
  ...
  assert.equal(
    ...
    "The 2026 Gulf Air Bahrain Grand Prix at Bahrain International Circuit was cancelled and no sessions were run."
  );
  assert.equal(
